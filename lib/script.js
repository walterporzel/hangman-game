...
// hides input form and shows keyboard and reset button
...
// event listner on input feild
document.querySelector(".initial-input").addEventListener('click', function(evt){
    evt.preventDefault();
    word = document.querySelector("input").value.toLowerCase();
    toggleDisplay();
});
...
// function for keyboard event listeners
function type(evt){
    evt.preventDefault();
...
let status = function(){
    ...
    let result = letters.filter(ele =>document.querySelector("."+ele).style.background === "green")
